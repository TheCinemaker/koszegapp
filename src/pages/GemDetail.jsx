...
export default function GemDetail() {
  ...
  // ÚJ: Egy külön állapot az animáció vezérléséhez
  const [showContent, setShowContent] = useState(false); 

  useEffect(() => {
    setLoading(true);
    setShowContent(false); // Reseteljük minden ID váltáskor
    ...
    fetchHiddenGems()
      .then(data => {
        ...
        if (!found) {
          ...
          setGem(found);
          // Adatbetöltés után várunk egy kicsit az animáció végéig
          setTimeout(() => {
            setLoading(false);
            setShowContent(true); // Csak most mutatjuk a tartalmat
          }, 1500); // <<< Ennek az időnek meg kell egyeznie a GIF hosszával!
        }
      })
      .catch(err => {
