    ...
    <div className="min-h-screen pb-24 pt-0 px-4">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4 pt-2">
          <Link to="/" className="w-9 h-9 rounded-full bg-white/50 dark:bg-white/10 backdrop-blur-md flex items-center justify-center shadow-sm hover:scale-105 transition-transform">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredHotels.length > 0 ? (
            filteredHotels.map((hotel, index) => (
              <Link
                key={`${hotel.id}-${index}`} // Fix Duplicate Key Error by appending index
                to={`/hotels/${hotel.id}`}
              ...
              >
                ...
                <div className="relative h-40 overflow-hidden z-10">
                  ...
                  {/* Fallback Ghost in case of no image or error (using nextSibling trick above or condition) */}
                  <div className={`${hotel.image ? 'hidden' : 'flex'} absolute inset-0 bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-slate-800 dark:to-slate-900 items-center justify-center`}>
                ...
                <div className="p-4 flex-1 flex flex-col justify-between relative bg-white/30 dark:bg-white/5">
                  <div className="flex items-start gap-2 text-gray-600 dark:text-gray-300 text-xs font-medium mb-3">
                    <FaMapMarkerAlt className="shrink-0 mt-0.5 text-indigo-500" />
                    <span className="line-clamp-2">{hotel.address}</span>
                  </div>

                  <div className="flex items-center justify-end">
                    <span className="px-4 py-2 rounded-full bg-indigo-600 text-white text-[10px] font-bold uppercase tracking-wider shadow-lg shadow-indigo-500/30 group-hover:bg-indigo-500 transition-colors">
                      Megtekintés
                    </span>
                  </div>
                </div>
