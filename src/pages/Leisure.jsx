        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredList.length > 0 ? (
            filteredList.map((item, index) => (
              <div
                key={`${item.id}-${index}`} // Fix Duplicate Key Error
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden z-10">
                  ...
                  {/* Fallback Ghost */}
                  <div className={`${item.image ? 'hidden' : 'flex'} absolute inset-0 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-slate-800 dark:to-slate-900 items-center justify-center`}>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between relative bg-white/30 dark:bg-white/5">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/20">
                    ...
                    <Link
                      to={`/leisure/${item.id}`}
                      className="px-4 py-2 rounded-full bg-purple-600 text-white text-[10px] font-bold uppercase tracking-wider shadow-lg shadow-purple-500/30 group-hover:bg-purple-500 transition-colors"
                    >
                      Részletek
                    </Link>
                  </div>
