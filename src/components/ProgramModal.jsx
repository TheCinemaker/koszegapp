...
export default function ProgramModal({ onClose }) {
    ...
    const calculateTimeLeft = useCallback(() => { const now = new Date(); const festivalStart = new Date('2025-10-24T10:00:00'); const diff = festivalStart - now; return { days: Math.max(0, Math.floor(diff / (1000 * 60 * 60 * 24))), hours: Math.max(0, Math.floor((diff / (1000 * 60 * 60)) % 24)), minutes: Math.max(0, Math.floor((diff / 1000 / 60) % 60)), seconds: Math.max(0, Math.floor((diff / 1000) % 60)), isOver: diff < 0, }; }, []);
