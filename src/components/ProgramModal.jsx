/* --- FÁJL: ProgramModal.jsx (Végleges, Többnyelvű, JAVÍTOTT VISSZASZÁMLÁLÓKKAL) --- */
...
function safeParseISO(dateString) { if (!dateString) return null; try { const date = parseISO(dateString); return isValid(date) ? date : null; } catch (error) { console.error(`Hibás dátumformátum, nem sikerült feldgozni: "${dateString}"`, error); return null; } }
function useFavorites() { const [favorites, setFavorites] = useState(() => { try { const stored = localStorage.getItem('programFavorites'); return stored ? JSON.parse(stored) : []; } catch (error) { console.error("Hiba a kedvencek betöltésekor a localStorage-ből:", error); return []; } }); useEffect(() => { try { localStorage.setItem('programFavorites', JSON.stringify(favorites)); } catch (error) { console.error("Hiba a kedvencek mentésekor a localStorage-be:", error); } }, [favorites]); const toggleFavorite = (eventId) => { setFavorites(prev => prev.includes(eventId) ? prev.filter(id => id !== eventId) : [...prev, eventId]); }; return { favorites, toggleFavorite }; }
function EventCard({ event, onSelect, isFavorite, onToggleFavorite, userLocation }) { const { t, i18n } = useTranslation(); const currentLang = i18n.language; const cardClasses = "p-3 rounded-xl border-l-4 cursor-pointer hover:shadow-lg transition mb-2 " + (isFavorite ? "bg-yellow-100 dark:bg-yellow-900/40 border-yellow-500" : "bg-purple-100 dark:bg-purple-900/50 border-purple-500"); return ( <div className={cardClasses} onClick={() => onSelect(event)}> <div className="flex items-start justify-between"> <div className="flex-grow pr-2"> <p className="font-bold text-gray-900 dark:text-gray-100">{event.nev[currentLang] || event.nev.hu}</p> <div className="text-sm mt-1 text-gray-700 dark:text-gray-300 space-y-1"> <p>📍 {event.helyszin.nev[currentLang] || event.helyszin.nev.hu}</p> <p>🕘 {format(event.start, 'HH:mm')} – {format(event.end, 'HH:mm')}</p> </div> </div> <button onClick={(e) => { e.stopPropagation(); onToggleFavorite(event.id); }} className="p-2 text-2xl flex-shrink-0" aria-label={isFavorite ? 'Eltávolítás a kedvencekből' : 'Hozzáadás a kedvencekhez'}> {isFavorite ? <span className="text-yellow-500 transition-transform duration-200 transform hover:scale-125">★</span> : <span className="text-gray-400 transition-transform duration-200 transform hover:scale-125">☆</span>} </button> </div> {userLocation && event.helyszin?.lat && ( <a href={`https://www.google.com/maps/dir/?api=1&origin=${userLocation.lat},${userLocation.lng}&destination=${event.helyszin.lat},${event.helyszin.lng}&travelmode=walking`} target="_blank" rel="noopener noreferrer" className="block mt-2 text-sm font-semibold text-purple-700 underline hover:text-purple-900 dark:text-purple-300" onClick={e => e.stopPropagation()}> 🧭 {t('programModal.routePlanner')} </a> )} </div> ); }
function InfoModal({ onClose }) { const { t } = useTranslation(); return ( <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={onClose}> <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl max-w-md w-full p-6 relative animate-scale-in" onClick={(e) => e.stopPropagation()}> <button onClick={onClose} className="absolute top-3 right-4 text-2xl text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 transition">×</button> <h2 className="text-2xl font-bold text-purple-800 dark:text-purple-200 mb-4">ℹ️ {t('infoModal.title')}</h2> <ul className="space-y-4 text-gray-700 dark:text-gray-300"> <li className="flex items-start gap-3"> <span className="text-xl pt-1">🔴</span> <div><Trans i18nKey="infoModal.item1" components={{ strong: <strong /> }} /></div> </li> <li className="flex items-start gap-3"> <span className="text-xl pt-1">🗓️</span> <div><Trans i18nKey="infoModal.item2" components={{ strong: <strong /> }} /></div> </li> <li className="flex items-start gap-3"> <span className="text-xl pt-1">★</span> <div><Trans i18nKey="infoModal.item3" components={{ strong: <strong /> }} /></div> </li> <li className="flex items-start gap-3"> <span className="text-xl pt-1">🍇</span> <div><Trans i18nKey="infoModal.item4" components={{ strong: <strong /> }} /></div> </li> </ul> <div className="mt-6 text-center"> <button onClick={onClose} className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-6 rounded-full transition"> {t('infoModal.button')} </button> </div> </div> </div> ); }

// --- JAVÍTOTT COUNTDOWNTONEXT ---
...
export default function ProgramModal({ onClose }) {
    ...
    const calculateTimeLeft = useCallback(() => { const now = new Date(); const festivalStart = new Date('2025-09-26T17:00:00'); const diff = festivalStart - now; return { days: Math.max(0, Math.floor(diff / (1000 * 60 * 60 * 24))), hours: Math.max(0, Math.floor((diff / (1000 * 60 * 60)) % 24)), minutes: Math.max(0, Math.floor((diff / 1000 / 60) % 60)), seconds: Math.max(0, Math.floor((diff / 1000) % 60)), isOver: diff < 0, }; }, []);
    const evaluateEvents = useCallback(() => { if (events.length === 0) return; const now = new Date(); const today = events.filter(e => isSameDay(e.start, now)); const curr = today.filter(e => isBefore(e.start, now) && isAfter(e.end, now)); const nextToday = today.filter(e => isAfter(e.start, now)).sort((a, b) => a.start - b.start); setCurrentEvents(curr); if (nextToday.length > 0) { const nextGroup = nextToday.filter(e => e.start.getTime() === nextToday[0].start.getTime()); setNextEvents(nextGroup); } else { const allFutureEvents = events.filter(e => isAfter(e.start, now)).sort((a, b) => a.start - b.start); if (allFutureEvents.length > 0) { const nextGroup = allFutureEvents.filter(e => e.start.getTime() === allFutureEvents[0].start.getTime()); setNextEvents(nextGroup); } else { setNextEvents([]); } } }, [events]);
    useEffect(() => { async function loadData() { try { const programResponse = await fetch('/data/programok.json'); if (!programResponse.ok) throw new Error(`Hálózati hiba: ${programResponse.statusText}`); const dailyData = await programResponse.json(); if (!Array.isArray(dailyData)) throw new Error("A programok.json formátuma nem megfelelő (nem tömb)."); const allEvents = dailyData.flatMap(dayObject => (dayObject && Array.isArray(dayObject.esemenyek)) ? dayObject.esemenyek : []); if (allEvents.length === 0) { console.warn("Nem található egyetlen esemény sem a 'programok.json' fájlban."); setEvents([]); } else { const parsed = allEvents.map(p => { const start = safeParseISO(p.idopont); if (!start) return null; const end = safeParseISO(p.veg_idopont) || new Date(start.getTime() + 60 * 60000); return { ...p, start, end, kiemelt: !!p.kiemelt }; }).filter(Boolean); setEvents(parsed); } fetch('https://api.openweathermap.org/data/2.5/weather?q=Koszeg,HU&units=metric&appid=ebe4857b9813fcfd39e7ce692e491045').then(res => res.json()).then(data => { if(data.cod === 200) setWeatherData({ temp: Math.round(data.main.temp), description: data.weather[0].description, icon: data.weather[0].icon }); }); } catch (err) { console.error("Adatbetöltési hiba:", err); setError(err.message || "Ismeretlen hiba történt a programok betöltésekor."); } finally { setIsLoading(false); } } loadData(); if (navigator.geolocation) navigator.geolocation.getCurrentPosition(pos => setUserLocation({ lat: pos.coords.latitude, lng: pos.coords.longitude }), err => console.warn('Helymeghatározás hiba:', err.message)); if ('serviceWorker' in navigator) navigator.serviceWorker.register('/service-worker.js').catch(error => console.error('Service Worker regisztráció hiba:', error)); }, []);
        ...
        <>
            <div className="fixed inset-y-4 sm:inset-y-8 inset-x-2 sm:inset-x-0 z-[999] px-2 pb-4 pointer-events-none">
                <div className="max-w-3xl mx-auto flex flex-col h-full pointer-events-auto">
                    ...
                    {!timeLeft.isOver && ( <div className="sticky top-[58px] z-10 bg-purple-900/95 backdrop-blur-sm text-white text-center p-2 shadow-inner"> <span className="font-mono text-sm"> {t('programModal.countdownPrefix')}{' '} {t('programModal.countdownFormat', { days: timeLeft.days, hours: timeLeft.hours, minutes: timeLeft.minutes, seconds: timeLeft.seconds })} </span> </div> )}
                    ...
                    <div className="bg-purple-50 dark:bg-zinc-900 p-4 rounded-b-2xl shadow-lg flex-grow overflow-y-auto">
                        ...
                        {view === 'today' && (<>
                            ...
                            {nextEvents.length > 0 && <div className="mb-4 animate-fadein">
                                <h3 className="section-title border-teal-300 dark:border-teal-700 text-teal-800 dark:text-teal-200 flex items-baseline gap-2">
                                    ...
                                    {isSameDay(nextEvents[0].start, new Date()) ? ( <InlineCountdown targetDate={nextEvents[0].start} /> ) : ( <span className="text-sm font-normal">{getNextEventDayInfo()}</span> )}
                                </h3>
                            ...
                            {userLocation && mapFocusEvent && (
                                <div className="mt-6">
                                    <div className="mb-2 flex justify-between items-center">
                                        ...
                                        <a href={`https://www.google.com/maps/dir/?api=1&origin=${userLocation.lat},${userLocation.lng}&destination=${mapFocusEvent.helyszin.lat},${mapFocusEvent.helyszin.lng}&travelmode=walking`} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-purple-700 underline hover:text-purple-900 dark:text-purple-300 flex items-center gap-1"> <span>🧭</span> <span>{t('programModal.takeMeThere')}</span> </a>
                                    </div>
                        ...
                        {view === 'favorites' && (
                            <div className="animate-fadein">{favoriteEvents.length > 0 ? 
